  ...
  return (
    <div className="flex flex-col h-full bg-white shadow-sm border rounded-lg overflow-hidden">
      ...
      <div ref={containerRef} className="overflow-auto flex-1 custom-scrollbar relative">
        ...
        {/* Header Row (Sticky) */}
        <div className="flex border-b border-slate-200 bg-slate-50 sticky top-0 z-20 shadow-sm min-w-max">
          <div className="w-16 md:w-24 flex-shrink-0 p-2 border-r border-slate-200 flex flex-col items-center justify-center text-slate-700 bg-slate-50">
        ...
        {monthsData.map((monthData) => (
          <React.Fragment key={`${monthData.year}-${monthData.month}`}>
            {/* Month Separator */}
            {viewMode !== '1month' && (
              <div className="sticky top-[57px] z-10 bg-slate-100/90 backdrop-blur-sm border-b border-slate-200 p-2 text-sm font-bold text-slate-600 pl-4 shadow-sm min-w-max">
